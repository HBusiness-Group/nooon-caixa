...
interface AppState {
  ...
  /** Paga fatura total — debita na conta corrente e restaura limite do CC */
  payInvoiceFull: (invoiceId: string, ccAccountId: string, sourceAccountId: string) => Promise<void>

  /** Pagamento parcial — debita na conta corrente e restaura limite parcialmente */
  payInvoicePartial: (invoiceId: string, ccAccountId: string, sourceAccountId: string, paidAmount: number, juros: number) => Promise<void>

  /** Parcela a fatura: cria lançamentos na conta corrente e muda status */
  installInvoice: (invoiceId: string, ccAccountId: string, sourceAccountId: string, installments: number) => Promise<void>
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  updateAccountFiduciary: async (id, data) => {
    // Para CC/CE, initial_balance deve sempre espelhar o limite
    // para que a view account_balances calcule o current_balance corretamente
    const account = get().accounts.find(a => a.id === id)
    const extraFields: Record<string, unknown> = {}
    if (account) {
      if (isCreditCard(account.type) && data.credit_limit !== undefined) {
        extraFields.initial_balance = data.credit_limit ?? 0
      }
      if (isOverdraft(account.type) && data.overdraft_limit !== undefined) {
        extraFields.initial_balance = data.overdraft_limit ?? 0
      }
    }
    const payload = { ...data, ...extraFields }
    await supabase.from('accounts').update(payload as any).eq('id', id)
    await get().loadAccounts()  // re-fetch completo para garantir sincronia
  },
...
ensureInvoice: async (accountId, referenceMonth) => {
  ...
  payInvoiceFull: async (invoiceId, ccAccountId, sourceAccountId) => {
    ...
    const descricao = `Pagamento fatura ${invoice.reference_month}`

    // 1. Saída na conta corrente de origem (debita o dinheiro real)
    await supabase.from('transactions').insert({
      user_id: userId,
    ...
    await supabase.from('invoice_audit_log').insert({
      ...
      new_value: { status: 'PAGO', paid_amount: invoice.total_amount, source_account_id: sourceAccountId },
    } as any)
  ...
  payInvoicePartial: async (invoiceId, ccAccountId, sourceAccountId, paidAmount, juros) => {
    ...
    const novoPago = (invoice.paid_amount ?? 0) + paidAmount
    const novoStatus = novoPago >= invoice.total_amount ? 'PAGO' : 'PARCIAL'
    const today = new Date().toISOString().split('T')[0]
    const descricao = `Pagamento parcial fatura ${invoice.reference_month}`
    ...
    // 3. Juros (se houver) — saída na conta de origem
    if (juros > 0) {
      await supabase.from('transactions').insert({
        user_id: userId,
        account_id: sourceAccountId,
        type: 'expense',
        description: `Juros e multas fatura ${invoice.reference_month}`,
    ...
    await supabase.from('invoice_audit_log').insert({
      ...
      old_value: { status: invoice.status, paid_amount: invoice.paid_amount },
      new_value: { status: novoStatus, paid_amount: novoPago, source_account_id: sourceAccountId },
    } as any)
  ...
  installInvoice: async (invoiceId, ccAccountId, sourceAccountId, installments) => {
    const { userId, invoices } = get()
    if (!userId) return
    ...
    const totalRestante = invoice.total_amount - (invoice.paid_amount ?? 0)
    const valorParcela = Math.round((totalRestante / installments) * 100) / 100
    const today = new Date()

    // Cria N parcelas de saída na conta de origem (meses subsequentes)
    for (let i = 0; i < installments; i++) {
      const refDate = format(addMonths(today, i), 'yyyy-MM-dd')
      await supabase.from('transactions').insert({
        user_id: userId,
        account_id: sourceAccountId,
        type: 'expense',
        description: `Parcela ${i + 1}/${installments} fatura ${invoice.reference_month}`,
        category: 'pagamento_emprestimos',
        amount: valorParcela,
        status: 'planned',
        date: refDate,
    ...
    await supabase.from('invoice_audit_log').insert({
      ...
      new_value: { status: 'PARCELADO', installments, valor_parcela: valorParcela, source_account_id: sourceAccountId },
    } as any)
