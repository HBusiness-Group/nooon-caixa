...
export default function TransactionModal({ onClose, editTx }: Props) {
  const { accounts, currentAccountId, addTransaction, loadTransactions, loadAccounts, allCategories } = useAppStore()
  const isEdit = !!editTx

  // Monta lista agrupada: system categories agrupadas + custom categories no final
  const groupedCats = getCategoriesByGroup()
  const customCats = allCategories().filter(c => !SYSTEM_CATEGORIES.some(s => s.key === c.key))

  // Default: primeira categoria do primeiro grupo
  const defaultCat = groupedCats[0]?.categories[0]?.key || 'receita_pessoal'
  ...
  async function handleSave() {
    ...
    if (isEdit && editTx) {
      ...
      const { error } = await supabase.from('transactions').update({
        type: tipo, description: finalDesc, amount: parseFloat(valor),
        account_id: accountId, date, status, category, group_ref: groupRef,
      } as any).eq('id', editTx.id)
      if (!error) { await loadTransactions(); await loadAccounts() }
    } else {
      await addTransaction({
        account_id: accountId, description: desc.trim(), group_ref: groupRef,
        category, amount: parseFloat(valor), type: tipo, status, date,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/75">
      ...
      >
        ...
        <h2
          className="font-['Barlow_Condensed'] text-xl font-black mb-5 tracking-wide"
          style={{ color: '#e8f5e2' }}
        >
        ...
        {isOverdue && (
          <div
            ...
            style={{ background: 'rgba(255,179,64,0.08)', borderColor: 'rgba(255,179,64,0.25)' }}
          >
            <div className="text-xs font-semibold mb-2" style={{ color: '#ffc04d' }}>
              ⚠ Lançamento atrasado — informe a data real do pagamento
            </div>
            ...
            <input
              ...
              className={inputCls}
              style={{ ...inputStyle, borderColor: 'rgba(255,179,64,0.3)' }}
            />
          </div>
        )}
        ...
        <div className="mb-3">
          ...
          <input
            ...
            placeholder="Ex: Crédito iPhone 15 PRO"
            className={inputCls}
            style={inputStyle}
          />
        </div>
        ...
        {status === 'simulated' && (
          <div
            className="mb-3 px-3 py-2 rounded-lg border text-[11px]"
            style={{ background: 'rgba(192,132,252,0.07)', borderColor: 'rgba(192,132,252,0.25)', color: '#c084fc' }}
          >
            ◈ Este lançamento é uma simulação. Ele só reflete na projeção quando o modo Simulado estiver{' '}
            <strong>ATIVADO</strong> no Calendário.
          </div>
        )}
        ...
        <div className="grid grid-cols-2 gap-3 mb-3">
            ...
            >
              ...
              {groupedCats.map(({ group, categories }) => (
                <optgroup
                  ...
                  style={{ background: '#1c2a1f', color: '#7ab070' }}
                >
                  {categories.map(cat => (
                    <option key={cat.key} value={cat.key} style={{ background: '#223026', color: '#e8f5e2' }}>
                      {cat.icon} {cat.label}
                    </option>
                  ))}
                </optgroup>
              ...
              {/* categorias customizadas (sem grupo fixo) */}
              {customCats.length > 0 && (
                <optgroup label="✦ Minhas categorias" style={{ background: '#1c2a1f', color: '#7ab070' }}>
                  {customCats.map(cat => (
                    <option key={cat.key} value={cat.key} style={{ background: '#223026', color: '#e8f5e2' }}>
                      {cat.icon} {cat.label}
                    </option>
          ...
          <div>
            ...
            <input
              ...
              placeholder="HB MicroCred 01/12"
              className={inputCls}
              style={inputStyle}
            />
          </div>
        ...
        {!isEdit && (
          <div className="mb-5">
            <label className="block text-[10px] font-bold uppercase tracking-widest mb-2" style={{ color: '#7ab070' }}>
              Parcelamento
            </label>
            ...
            {installments === 0 && (
              <div className="flex items-center gap-2 mb-2">
                ...
                <input
                  ...
                  max={360}
                  value={customParc}
                  onChange={e => setCustomParc(e.target.value)}
                  placeholder="Ex: 72"
