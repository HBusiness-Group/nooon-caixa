'use client'
// src/components/fiduciary/InvoicePanel.tsx — NOOON Caixa V4 #002
// Painel de gestão de fatura para Cartão de Crédito e Cheque Especial
...
export default function InvoicePanel({ accountId, onClose }: Props) {
  ...
  const [interestVal, setInterestVal] = useState('')   // CE: juros obrigatório
  ...
  async function handlePayFull() {
    ...
    // CE: juros obrigatório
    if (isCE && !interestVal) return

    setLoading(true)
    if (isCE && interestVal) {
      await setInvoiceInterest(invoice.id, parseFloat(interestVal))
    }
  ...
  // ── Helpers de exibição ─────────────────────────────────────────────────
  const remaining   = invoice ? invoice.total_amount - invoice.paid_amount : 0
  const isPending   = invoice && invoice.status !== 'PAGO'
  const limitUsed   = isCC && account?.credit_limit
    ? ((invoice?.total_amount ?? 0) / account.credit_limit) * 100
    : null
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/75">
      ...
      >
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            <div className="flex items-center gap-2">
              <span style={{ fontSize: 20 }}>{isCC ? '💳' : '🏦'}</span>
              <h2
                className="font-['Barlow_Condensed'] text-xl font-black tracking-wide"
                style={{ color: '#e8f5e2' }}
              >
                {account.name}
              </h2>
        ...
        {/* Alerta visual: obrigação pendente */}
        {isPending && (
          <div
          ...
          >
            ...
            <div className="text-[12px] font-semibold" style={{ color: '#ff6b6b' }}>
              ...
              <strong>{invoice ? format(parseISO(invoice.due_date), "dd/MM/yyyy", { locale: ptBR }) : '—'}</strong>
            </div>
          </div>
        )}
        ...
        {tab === 'fatura' && invoice && (
          <div>
            ...
            <div className="grid grid-cols-3 gap-2 mb-5">
              {[
                { label: 'Total fatura', value: invoice.total_amount, color: '#ff6b6b' },
                { label: 'Pago',         value: invoice.paid_amount,  color: '#6dd400' },
            ...
            {/* Barra de limite (CC) */}
            {isCC && account.credit_limit && limitUsed !== null && (
              <div className="mb-5">
                <div className="flex justify-between text-[10px] mb-1.5" style={{ color: '#7ab070' }}>
                  <span>Limite utilizado</span>
                  <span style={{ color: limitUsed > 80 ? '#ff6b6b' : '#a8c8a0' }}>
                    {fmtCurrency(invoice.total_amount)} / {fmtCurrency(account.credit_limit)} ({limitUsed.toFixed(0)}%)
                  </span>
                </div>
                <div className="w-full h-2 rounded-full overflow-hidden" style={{ background: '#2a3a2e' }}>
                  <div
                    className="h-2 rounded-full transition-all"
                    style={{
                      width: `${Math.min(limitUsed, 100)}%`,
                      background: limitUsed > 80 ? '#ff6b6b' : limitUsed > 50 ? '#ffb340' : '#6dd400',
                    }}
                  />
                </div>
              </div>
            ...
            {/* Datas */}
            <div className="grid grid-cols-2 gap-2 mb-5 text-xs" style={{ color: '#a8c8a0' }}>
              <div className="rounded-lg px-3 py-2 border" style={{ background: '#223026', borderColor: 'rgba(109,212,0,0.1)' }}>
                ...
                <div className="font-semibold">{format(parseISO(invoice.close_date), "dd/MM/yyyy")}</div>
              </div>
              <div className="rounded-lg px-3 py-2 border" style={{ background: '#223026', borderColor: 'rgba(109,212,0,0.1)' }}>
                <div className="text-[10px] uppercase tracking-wider mb-0.5" style={{ color: '#7ab070' }}>Vencimento</div>
                <div className="font-semibold">{format(parseISO(invoice.due_date), "dd/MM/yyyy")}</div>
              </div>
            </div>
            ...
            {invoice.generates_interest && (
              <div
                className="mb-4 px-3 py-2.5 rounded-lg border text-xs"
                style={{ background: 'rgba(217,70,239,0.07)', borderColor: 'rgba(217,70,239,0.25)', color: '#d946ef' }}
              >
                ⚠️ Esta fatura gerará <strong>Juros e Multas</strong> na próxima — ajuste o valor do marcador nos registros.
              </div>
            )}

            {/* Botões de ação (só se não pago) */}
            {invoice.status !== 'PAGO' && !action && (
              <div className="grid grid-cols-1 gap-2">
                <button
                  onClick={() => setAction('pago')}
                  className="w-full py-3 rounded-xl font-['Barlow_Condensed'] font-black text-sm uppercase tracking-wider border transition-all"
            ...
            {/* ── Formulário: PAGO ────────────────────────────────────────── */}
            {action === 'pago' && (
              <div className="rounded-xl border p-4" style={{ background: '#223026', borderColor: 'rgba(109,212,0,0.2)' }}>
                ...
                {isCE && (
                  <div className="mb-3">
                    <label className={labelCls} style={{ color: '#d946ef' }}>
                      ⚠️ Valor de Juros e Multas cobrados (obrigatório)
                    </label>
                    <input
                      type="number"
                      value={interestVal}
                      onChange={e => setInterestVal(e.target.value)}
                      placeholder="Ex: 47,80"
                      className={inputCls}
                ...
                <div className="flex gap-2 mt-3">
                  <button
                    ...
                    disabled={loading || (isCE && !interestVal)}
                  ...
                  <button onClick={() => setAction(null)} className="px-4 py-2.5 rounded-lg text-sm border" style={{ borderColor: 'rgba(255,255,255,0.1)', color: '#7ab070' }}>
                    Cancelar
                  </button>
                </div>
              </div>
            ...
            {/* ── Formulário: PARCIAL ─────────────────────────────────────── */}
            {action === 'parcial' && (
              <div className="rounded-xl border p-4" style={{ background: '#223026', borderColor: 'rgba(255,179,64,0.2)' }}>
                ...
                <div className="mb-3">
                  ...
                  <input type="number" value={payValue} onChange={e => setPayValue(e.target.value)} placeholder="0,00" className={inputCls} style={inputStyle} />
                </div>
                <div className="mb-3">
                  <label className={labelCls} style={{ color: '#7ab070' }}>Data do pagamento</label>
                  <input type="date" value={payDate} onChange={e => setPayDate(e.target.value)} className={inputCls} style={inputStyle} />
                ...
                {payValue && parseFloat(payValue) > 0 && (
                  <div className="text-xs mb-3 px-3 py-2 rounded-lg border" style={{ background: '#1c2a1f', borderColor: 'rgba(255,179,64,0.15)', color: '#a8c8a0' }}>
                    Restante: <strong style={{ color: '#ff6b6b' }}>{fmtCurrency(invoice.total_amount - parseFloat(payValue))}</strong>
                    {' '}· Um marcador de juros R$1,00 será criado na próxima fatura.
                  </div>
                )}

                <div className="flex gap-2 mt-3">
                  <button onClick={handlePayPartial} disabled={loading || !payValue} className="flex-1 py-2.5 rounded-lg text-sm font-bold uppercase tracking-wider disabled:opacity-40" style={{ background: '#ffb340', color: '#1a1a1a' }}>
                    {loading ? 'Salvando...' : 'Confirmar'}
                  </button>
                  <button onClick={() => setAction(null)} className="px-4 py-2.5 rounded-lg text-sm border" style={{ borderColor: 'rgba(255,255,255,0.1)', color: '#7ab070' }}>
                    Cancelar
            ...
            {action === 'parcelado' && (
              <div className="rounded-xl border p-4" style={{ background: '#223026', borderColor: 'rgba(91,200,255,0.2)' }}>
                ...
                {payValue && parseFloat(payValue) > 0 && (
                  <div className="text-xs mb-3 px-3 py-2 rounded-lg border" style={{ background: '#1c2a1f', borderColor: 'rgba(91,200,255,0.15)', color: '#a8c8a0' }}>
                    ...
                    <span style={{ color: '#5bc8ff', fontWeight: 700 }}>
                      {fmtCurrency(parseFloat(payValue) / installN)}
                    </span>
                ...
                <div className="flex gap-2 mt-3">
                  <button onClick={handleInstall} disabled={loading || !payValue} className="flex-1 py-2.5 rounded-lg text-sm font-bold uppercase tracking-wider disabled:opacity-40" style={{ background: '#5bc8ff', color: '#0a1a2a' }}>
                    {loading ? 'Salvando...' : 'Confirmar'}
                  </button>
                  <button onClick={() => setAction(null)} className="px-4 py-2.5 rounded-lg text-sm border" style={{ borderColor: 'rgba(255,255,255,0.1)', color: '#7ab070' }}>
                    Cancelar
